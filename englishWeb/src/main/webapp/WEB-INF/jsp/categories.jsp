...
<html>
  <head>
      ...
      <script src="js/english.js"></script>
            <script>
                function next(){
                    if(${fullList!=true}){
                        window.location = "/categories.html?portion=${portion}&startPosition=${startPosition+portion}"
                    }
                }
                function preview(){
                    if(${startPosition>0}){
  ...
  <div id="mainBlock">
      <form class="formWork" onsubmit="return checkForEmpty('inputParam','error')" action="/categories.html" method="post">
          <a class="promptText"> Choice amount roles to view: </a>

          <c:if test="${portion==null}">
              <input class="inputParam" id="range" type="range" min="5" max="20" step="5" name="portion"
                     onchange="rangeValue('range','rangeValue')" value="10" > <div class="error" ></div >
              <a id="rangeValue">10</a>
          </c:if>

          <c:if test="${portion!=null}">
              <input class="viewUsers" id="range" type="range" min="5" max="20" step="5" name="portion"
      ...
      <h4>${userName}, where are categories by portion: </h4>
